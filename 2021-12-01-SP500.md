...
const performance = lev => {
  window.levgrowth = changes.map((change, i) => 1 + lev * change - (lev - 1) * sp[i]["Long Interest Rate"] / 1200).reduce((a, b) => a.concat(a[a.length - 1] * b), [1]);
  results = data => data.slice(-860).map((index, i, arr) => ({x: sp[sp.length - 860 + i].Date, y: index / arr[0]}));
