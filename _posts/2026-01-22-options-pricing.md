...
Assume one takes steps of equal size \\( s \\) in a fixed interval \\( \tau \\), with the direction of each step decided randomly between noth and south. After the time \\( t \\) there will have been \\( \frac{t}{\tau} \\) steps made, and by the [central limit theorem](https://en.wikipedia.org/wiki/Central_limit_theorem) for small \\( \tau \\), the probability of beeing at point \\( x(t) \\) at time \\( t \\) will follow a normal distribution with expectation value \\( \mathbb{E}[x] = \frac{t}{\tau} (p_\uparrow-p_\downarrow) s =: \mu t \\) and variance \\( \mathbb{V}[x] = \frac{t}{\tau} s^2 =: \sigma^2 t \\).

Taking the limit \\( \tau \rightarrow 0 \\) while keeping \\( \mu \\) and \\( \sigma^2 \\) constant results in a so-called Wiener process. A change \\( \Delta x \\) after time \\( \Delta t\\) is then again drawn from a normal distribution \\( \Delta x \sim \mathcal{N}(\Delta t \mu, \Delta t \sigma^2) \sim \mu\Delta t + \sigma\sqrt{\Delta t}\,\mathcal{N}(0, 1) \\), making the derivative \\( \frac{\Delta x}{\Delta t} \sim \mu + \frac{\sigma}{\sqrt{\Delta t}}\mathcal{N}(0, 1) \\) divergent in the limit \\( \Delta t \rightarrow 0 \\). Therefore the function \\( x(t) \\) is continous everywhere but differentiable nowhere.

### Itô's lemma

If one now assumes that the price of a stock is governed by a Wiener process and the option price \\( V(S, t)\\) depends only on the stock price \\( S \\) and time \\( t \\), with all other parameters \\( \mu, \sigma, r \\) assumed to be constant, then one can look at the change \\( \Delta V(S, t) \\) after the time \\( \Delta t \\) by using the Taylor expansion of \\( V \\):
...
This model was first described by Bachelier in 1900, predating Black and Scholes by seven decades. The resulting differential equation is then given by:
