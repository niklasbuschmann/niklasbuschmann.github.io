...
The four probability densities are then:
 - the **posterior** \\( \textrm{p}(\theta\|x) \\) which is the probability of the parameter being \\( \theta \\) considering that \\( x \\) was measured
 - the **likelihood** \\( \textrm{p}(x\|\theta) \\) which is the probability of measuring \\( x \\) for a given parameter \\( \theta \\)
...
$$
  \textrm{p}(\mathbf{x}|\theta) = \prod_i \textrm{p}(x_i|\theta) 
  = e^{\log(\prod_i \textrm{p}(x_i|\theta))}
  = e^{\sum_i \log(\textrm{p}(x_i|\theta))}
$$
...
$$
  \sum_i \log(\textrm{p}(x_i|\theta)) \approx \sum_i \log(\textrm{p}(x_i|\theta_{\textrm{ML}})) + \left(\theta-\theta_{\textrm{ML}}\right)^2\frac{\partial^2}{\partial \theta^2}\sum_i \log(\textrm{p}(x_i|\theta_{\textrm{ML}}))/2
$$
...
Instead of stating the full posterior \\( \textrm{p}(\theta\|\mathbf{x}) \\), one can characterise the distribution by a value \\( \hat{\theta}(\mathbf{x}) \\) that minimizes on average a loss function \\( \int L(\hat{\theta}(\mathbf{x}),\theta)\textrm{p}(\theta\|\mathbf{x})\textrm{d}\theta \\) over the posterior \\( \textrm{p}(\theta\|x) \\). Possible loss functions include:
...
The likelihood function \\( \textrm{p}(x\|\mu,\sigma) \\) can be written with \\( \overline{x} = \sum_i \frac{x_i}{n} \\) as:

$$
  \textrm{p}(\mathbf{x}|\mu,\sigma) = \prod_i \textrm{p}(x_i|\mu,\sigma) = \prod_i \frac{e^{-\frac{(x_i-\mu)^2}{2\sigma^2}}}{\sqrt{2\pi \sigma^2}}
...
Using the Jeffreys prior \\( \textrm{p}(\mu) = 1 \\), \\( \textrm{p}(\sigma) = \frac{1}{\sqrt{2\pi\sigma^2}} \\) and integrating out \\( \mu \\) and \\( \sigma \\) yields:
...
A way of parameter estimation is the maximum likelihood method where the estimator \\( \hat{\theta}(\mathbf{x}) \\) is given by the condition that \\( \hat{\theta}(\mathbf{x}) \\) maximizes the likelihood, implying \\( \frac{\partial \textrm{p}(\mathbf{x}\|\hat{\theta}(\mathbf{x}))}{\partial \theta} \overset{!}{=} 0 \\). One can Taylor expand the likelihood around the true parameter \\( \theta_0 \\):

$$
  \frac{\partial \log(\textrm{p}(\mathbf{x}|\theta_0))}{\partial \theta} \approx \underbrace{\frac{\partial \log(\textrm{p}(\mathbf{x}|\hat{\theta}(\mathbf{x})))}{\partial \theta}}_{0} + \frac{\partial^2 \log(\textrm{p}(\mathbf{x}|\hat{\theta}(\mathbf{x})))}{\partial \theta^2} (\theta_0-\hat{\theta}(\mathbf{x}))
$$
...
$$
   \theta_0-\hat{\theta}(\mathbf{x}) \ \overset{n\rightarrow\infty}{\rightarrow}\  \mathcal{N}\left(\mu = 0, \sigma^2 = \frac{1}{n I(\theta)}\right)
$$
