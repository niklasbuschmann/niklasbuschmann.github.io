...
const performance = months => {
  ...
  return levgrowth.map((data, i) => ({name: i + 'x monthly leveraged', data: results(months, data)}));
}

const cagr = years => {
  const results = (years, data) => sp.map((item, i) => i).slice(-12 * (50)).map(i => ({x: sp[i].Date, y: (data[i] / data[i - years * 12]) ** (1 / years)}));
  return levgrowth.map((data, i) => ({name: i + 'x monthly leveraged', data: results(years, data)}));
}
...
const dca = years => {
  ...
  return [1, 12, 24, 48].map(months => ({name: months + ' months average', data: results(years, levgrowth[0], months)}));
}
